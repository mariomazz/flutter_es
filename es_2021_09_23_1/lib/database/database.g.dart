...
class _$EmployeeDao extends EmployeeDao {
  ...
  Stream<List<Employee>> deleteAllEmployee() {
    return _queryAdapter.queryListStream('DELETE * FROM Employee',
        mapper: (Map<String, Object?> row) => Employee(
            id: row['id'] as int,
            firstName: row['firstName'] as String,
            lastName: row['lastName'] as String,
  ...
  Stream<List<Employee>> deleteEmployeeById(int id) {
    return _queryAdapter.queryListStream('DELETE * FROM Employee WHERE id=?1',
        mapper: (Map<String, Object?> row) => Employee(
            id: row['id'] as int,
            firstName: row['firstName'] as String,
            lastName: row['lastName'] as String,
  ...
  @override
  Future<void> insertEmployee(Employee employees) async {
    await _employeeInsertionAdapter.insert(employees, OnConflictStrategy.abort);
  }

  @override
  Future<void> updateEmployee(Employee employees) async {
    await _employeeUpdateAdapter.update(employees, OnConflictStrategy.abort);
  }

  @override
  Future<void> deleteEmployee(Employee employees) async {
    await _employeeDeletionAdapter.delete(employees);
  }
}
